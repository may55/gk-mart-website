...
import { ChevronRight, Headphones, LogOut, MapPin } from "lucide-react";
...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Manage your GK Mart profile and delivery addresses." },
...
const preferences = [{ icon: Headphones, label: "Help & support" }];
        ...
        <main className="flex-1 px-5">
          ...
          <div className="flex items-center gap-4 rounded-2xl border border-border bg-card p-4 shadow-[var(--shadow-card)]">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-[11px] font-semibold text-primary">GK Gold Member</p>
            </div>
            <button
              type="button"
          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-card)]">
            {preferences.map((item, i) => (
              <button
                ...
                type="button"
                onClick={() => {
                  if (item.label === "Help & support") navigate({ to: "/help-support" });
                }}
              ...
              >
                ...
                <p className="min-w-0 flex-1 truncate text-sm font-semibold">{item.label}</p>
                <ChevronRight className="h-4 w-4 text-muted-foreground" />
              </button>
            ))}
          </div>
