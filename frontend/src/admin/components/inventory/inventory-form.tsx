...
export function InventoryForm({ batch, onClose, onSaved }: Props) {
  const isEdit = !!batch;
  const [items, setItems] = useState<Item[]>([]);
  const [itemEnum, setItemEnum] = useState(batch?.itemEnum ?? "");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-xl border border-border bg-card shadow-xl">
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          <h2 className="text-lg font-semibold">
            {isEdit ? "Update Margin" : "Add Inventory Batch"}
          </h2>
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground">
            <X className="h-5 w-5" />
          </button>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-6">
          {isEdit ? (
            ...
            <div className="space-y-1.5">
              ...
              <select
                value={itemEnum}
                onChange={(e) => setItemEnum(e.target.value)}
                required
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
                <option value="">Select an item...</option>
                {items.map((item) => (
                  <option key={item.enum} value={item.enum}>
                    {item.name} {item.volume} ({item.enum})
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="space-y-1.5">
            ...
            {isEdit && (
              <p className="text-xs text-muted-foreground">
                Updating margin will recalculate the item's selling price
              </p>
            )}
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              {isLoading && <Loader2 className="h-4 w-4 animate-spin" />}
              {isEdit ? "Update Margin" : "Create Batch"}
            </button>
          </div>
        </form>
      </div>
