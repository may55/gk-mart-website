...
interface Address { label: string; line1: string; line2: string; pincode: string; city: string; state: string; phone: string; }
...
const EMPTY_ADDRESS: Address = { label: "", line1: "", line2: "", pincode: "", city: "", state: "", phone: "" };
        ...
        <form onSubmit={handleSubmit} className="max-h-[80vh] overflow-y-auto p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <select value={userRole} onChange={(e) => setUserRole(e.target.value as "admin" | "customer")}
                className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring">
              ...
              <div key={i} className="rounded-lg border border-border p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {([
                    ...
                    ["pincode", "Pincode"],
                    ["state", "State"],
                  ] as [keyof Address, string][]).map(([k, displayLabel]) => (
                    <div key={k} className={`space-y-1 ${
                      k === "line1" || k === "line2" ? "col-span-2" : ""
                    }`}>
                      <p className="text-xs text-muted-foreground">{displayLabel}</p>
                      <input value={addr[k]} onChange={(e) => setAddr(i, k, e.target.value)}
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose}
              className="rounded-md border border-border px-4 py-2 text-sm font-medium hover:bg-accent">Cancel</button>
